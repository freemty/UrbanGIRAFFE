...
import numpy as np
def get_camera_frustum(img_size, K, W2C, frustum_length=0.5, color=[0., 1., 0.], z_reverse = False):
    W, H = img_size
    ...
    frustum_points = np.array([[0., 0., 0.],                          # frustum origin
                               [-half_w, -half_h, frustum_length * (1 - 2 * z_reverse  )],    # top-left image corner
                               [half_w, -half_h, frustum_length * (1 - 2 * z_reverse  )],     # top-right image corner
                               [half_w, half_h, frustum_length* (1 - 2 * z_reverse  )],      # bottom-right image corner
                               [-half_w, half_h, frustum_length* (1 - 2 * z_reverse  )]])    # bottom-left image corner

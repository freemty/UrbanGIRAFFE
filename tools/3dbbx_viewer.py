...
# annotation helper
from tools.kitti360Scripts.helpers.annotation  import Annotation3D, Annotation3DPly, global2local
from tools.kitti360Scripts.helpers.project     import Camera
from tools.kitti360Scripts.helpers.labels      import name2label, id2label, kittiId2label
...
class Kitti360Viewer3D(object):
    ...
    # Constructor
    def __init__(self, path = 'datasets/KITTI-360', seq=0, showStatic=True, load_full = False):
        ...
        self.annotation3D = Annotation3D(self.label3DBboxPath, sequence, load_full=load_full)
        ...
        self.bboxes = []
        self.bboxes_window = []
        self.bboxes_semanticId = []
        self.bboxes_globalId = []
        self.accumuData = []
    ...
    def loadBoundingBoxes(self):
        ...
        for globalId,v in self.annotation3D.objects.items():
            ...
            for obj in v.values():
                ...
                self.bboxes_semanticId.append(semanticId)
                self.bboxes_globalId.append(globalId)
...
if __name__=='__main__':
    ...
    parser.add_argument('--max_bbox', type=int, default=1000,
                                help='The maximum number of bounding boxes to visualize')
    #parser.add_argument('show_all_inatence', action='store_false',default=False)   

    show_all_inatence = True
    args = parser.parse_args()

    visible_semanticId = [6,7,8,9,10, 22]

    v = Kitti360Viewer3D(path='datasets/KITTI-360', seq=args.sequence, load_full=True)
    v.loadBoundingBoxes()

        # group the bboxes by windows
    windows_unique = np.unique(np.array(v.bboxes_window), axis=0)
    for window in windows_unique:
        if show_all_inatence:
            bboxes = [v.bboxes[i] for i in range(len(v.bboxes)) if v.bboxes_window[i][0]==window[0]]
        else:
            bboxes = [v.bboxes[i] for i in range(len(v.bboxes)) if v.bboxes_window[i][0]==window[0] and v.bboxes_semanticId[i] in visible_semanticId]

...
class KITTI360Bbox3D(KITTI360Object):
    ...
    def parseBbox(self, child, load_full = False):
...
class Annotation2D:
    # Constructor
    def __init__(self, colormap='Set1', N = 1000):
...
class Annotation2DInstance:
    def __init__(self, gtPath,N = 1000, cam=0):
...
class Annotation3D:
    # Constructor
    def __init__(self, labelDir='', sequence='', load_full = False):

        self.load_full = load_full
        if not load_full:
            labelPath = os.path.join(labelDir, 'train', '%s.xml' % sequence)
        else:
            labelPath = os.path.join(labelDir, 'train_full', '%s.xml' % sequence)
    ...
    def init_instance(self, labelPath):
        ...
        self.global2ann = defaultdict(dict)
        self.ann2global = defaultdict(dict)
        for child in root:
            if child.find('transform') is None:
                continue
            obj = KITTI360Bbox3D()
            obj.parseBbox(child, load_full = self.load_full)
            
            globalId = local2global(obj.semanticId, obj.instanceId)
            self.objects[globalId][obj.timestamp] = obj
            self.global2ann[globalId] = obj.annotationId
            self.ann2global[obj.annotationId] = globalId

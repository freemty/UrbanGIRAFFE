...
std::vector<torch::Tensor> ray_voxel_intersection_perspective_cuda(
    ...
    float cam_f, 
    const std::vector<float>& cam_c, 
    const std::vector<int>& img_dims, 
    int max_samples);


// World Coordinate Sparse Trilinear Interpolation
torch::Tensor sp_trilinear_worldcoord_cuda(
    const torch::Tensor& in_feature, 
    const torch::Tensor& in_corner_lut, 
    const torch::Tensor& in_worldcoord, 
    bool ign_zero, 
    int channel_pos);

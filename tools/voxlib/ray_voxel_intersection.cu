...
static __global__ void ray_voxel_intersection_perspective_kernel(
    int32_t* __restrict__ out_voxel_id, 
    float* __restrict__ out_depth, 
    float* __restrict__ out_raydirs,
const int32_t* __restrict__ in_voxel, 
const float* __restrict__ in_raydirs, 
const RVIP_Params p) {
    ...
    ndc_imcoords[0] = (float)img_coords[0] - p.cam_c[0]; 
    ndc_imcoords[1] = (float)img_coords[1] - p.cam_c[1];

    raydir[0] = in_raydirs[pix_index*3];
    raydir[1] = in_raydirs[pix_index*3+1];
    raydir[2] = in_raydirs[pix_index*3+2];
    ...
    // Save out_raydirs
    out_raydirs[pix_index*3] = in_raydirs[pix_index*3];
    out_raydirs[pix_index*3+1] = in_raydirs[pix_index*3+1];
    out_raydirs[pix_index*3+2] = in_raydirs[pix_index*3+2];
...
/*
    ...
    in:
        ...
        in_raydirs:  torch CUDA float  [   img_dims[0], img_dims[1],3]
...
const torch::Tensor& in_voxel,
const torch::Tensor& in_raydirs,
const torch::Tensor& cam_ori, 
const torch::Tensor& cam_dir, 
const torch::Tensor& cam_up, 
float cam_f, const std::vector<float>& cam_c, 
const std::vector<int>& img_dims, 
int max_samples) {
    CHECK_CUDA(in_voxel);
    CHECK_CUDA(in_raydirs);
    ...
    assert(in_raydirs.dtype() == torch::kFloat32); // Minecraft compatibility
    assert(in_raydirs.dim() == 3);
    ...
    ray_voxel_intersection_perspective_kernel<TILE_DIM><<<dimGrid, dimBlock, 0, stream>>>(
        out_voxel_id.data_ptr<int32_t>(), 
        out_depth.data_ptr<float>(), 
        out_raydirs.data_ptr<float>(), 
        in_voxel.data_ptr<int32_t>(), 
        in_raydirs.data_ptr<float>(), 
        p
    );

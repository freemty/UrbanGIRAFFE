...
def filter_gt(pan_gt, gt_ann):
    ...
    for label, label_cnt in zip(labels, labels_cnt):
        if (label == 17000) or (label == 20000) or (label == 38000) or (int(label/1000) == 33) or (label == 6000):
            pan_gt[pan_gt == label] = VOID
            remove_id.append(label)
        elif (int(label/1000) == 11) and (label_cnt < 32 * 32):
            pan_gt[pan_gt == label] = VOID
            remove_id.append(label)
        elif (int(label/1000) == 30) and (label_cnt < 32 * 32):
            pan_gt[pan_gt == label] = VOID
            remove_id.append(label)
        elif (int(label/1000) == 26) and (label_cnt < 400):
            pan_gt[pan_gt == label] = VOID
            remove_id.append(label)
        elif (int(label/1000) == 41) and (label_cnt < 400):
            pan_gt[pan_gt == label] = VOID
            remove_id.append(label)
        elif  (label_cnt < 100):
            pan_gt[pan_gt == label] = VOID
...
def filter_pred(pan_pred, pred_ann):
    ...
    remove_id = []
    for label, label_cnt in zip(labels, labels_cnt):
        if (label == 17) or (label == 20) or (label == 38) or (label == 33) or (label == 6)  or (label == 0) or (label == 50):
...
def pq_compute_single_core(proc_id, annotation_set, categories):
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        pan_pred = np.load(pred_ann['path'])
        ...
        for label_tuple, intersection in gt_pred_map.items():
            ...
            if (iou > 0.5):# or (gt_segms[gt_label]['isthing'] == True):
...
def pq_compute(gt_json_file, pred_json_file):
    ...
    with open(os.path.join(cfg.result_dir,'match_set.json'), 'w') as fp:
...
def evaluate():
    ...
    for path in path_list:
        with open(os.path.join(path, 'gt.json'), 'r') as f:
            gt_json_per = json.load(f)
        with open(os.path.join(path, 'pred.json'), 'r') as f:
            pred_json_per = json.load(f)
        for i in range(len(gt_json_per)):
            gt_folder = 'datasets/KITTI-360/gt_2d_panoptics/seq_0000/instance'
            gt_json_per[i]['path'] = os.path.join(gt_folder, gt_json_per[i]['file_name'])
        for i in range(len(pred_json_per)):
            pred_json_per[i]['path'] = os.path.join(path, '2', pred_json_per[i]['file_name'])
        gt_json += gt_json_per

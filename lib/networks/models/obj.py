...
from .layers import PositionalEncoding, EqualLinear, ModulationLinear
...
class giraffeDecoder(nn.Module):
    def __init__(self,
        ...
        **kwargs):
        ...
        self.pe_pts = PositionalEncoding(in_dim=3, frequency_bands=n_freq_posenc_pts, include_input=True)
        self.pe_c = PositionalEncoding(in_dim=c_dim, frequency_bands=n_freq_posenc_c, include_input=True)
        self.pe_view = PositionalEncoding(in_dim=3, frequency_bands=n_freq_posenc_views,include_input=True)

        pts_dim_embed = self.pe_pts.out_dim
        c_dim_embed = self.pe_c.out_dim
        view_dim_embed = self.pe_view.out_dim

        self.block_num = 8
        self.skip = 4
        
        # Density Prediction Layers
        self.fc_in = EqualLinear(pts_dim_embed, hidden_size)

        if z_dim > 0:
            self.fc_z = EqualLinear(z_dim, hidden_size)
        if c_dim > 0:
            self.fc_c = EqualLinear(c_dim_embed, hidden_size)


        blocks = [EqualLinear(hidden_size, hidden_size, activate=True) for i in range(n_blocks - 1)]
        self.blocks = nn.Sequential(*blocks)
        n_skips = sum([i in skips for i in range(n_blocks - 1)])
        if n_skips > 0:
            self.fc_z_skips = nn.ModuleList(
                [EqualLinear(z_dim, hidden_size) for i in range(n_skips)]
            )
            self.fc_p_skips = nn.ModuleList([
                EqualLinear(pts_dim_embed, hidden_size) for i in range(n_skips)
            ])
            self.fc_c_skips = nn.ModuleList([
                EqualLinear(c_dim_embed, hidden_size) for i in range(n_skips)
            ])
        self.sigma_out = EqualLinear(hidden_size, 1, activate= False)

        # Feature Prediction Layers
        self.fc_z_view = EqualLinear(z_dim, hidden_size)
        self.feat_view = EqualLinear(hidden_size, hidden_size)
        self.fc_view =EqualLinear(view_dim_embed, hidden_size)
        self.feat_out = EqualLinear(hidden_size, out_channel)

        if use_viewdirs and n_blocks_view > 1:
            self.blocks_view = nn.ModuleList(
                [EqualLinear(view_dim_embed + hidden_size, hidden_size)
    ...
    def forward(self, pts,z, c = None, ray_d = None,**kwargs):
        ...
        for idx, layer in enumerate(self.blocks):
            ...
            if (idx + 1) in self.skips and (idx < len(self.blocks) - 1):
                ...
                net = net + self.fc_p_skips[skip_idx](pts)
                if self.c_dim > 0:
...
class giraffeDecoder_legacy(nn.Module):
    ''' Decoder class.

...
class StyleGAN2Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        sh = self.h_refine // (2 ** np.log2(resolution / init_res))
        self.h_init, self.w_init = int(sh), int(hw_ratio * sh)
        ...
        for res_log2 in range(self.final_res_log2, self.init_res_log2 - 1, -1):
            ...
            # Convolution block for last resolution.
                ...
                layer_name = f'layer{2 * block_idx + 1}'
                self.add_module(layer_name,
                                DenseLayer(in_channels=in_channels * self.h_init * self.w_init,
                                           out_channels=in_channels,
                                           add_bias=True,
                                           init_bias=0.0,
                                           use_wscale=use_wscale,

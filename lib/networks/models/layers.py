...
from .utils.gsn_ops import upfirdn2d, FusedLeakyReLU, fused_leaky_relu
from .utils.torch_utils.ops.conv2d_gradfix import conv2d, conv_transpose2d
# from .utils.torch_utils.ops.upfirdn2d import upfirdn2d
...
class EqualConv2d(nn.Module):
    ...
    def forward(self, input):
        if True:
            out = conv2d(input, self.weight * self.scale, bias=self.bias, stride=self.stride, padding=self.padding)
        else:
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, z):
        ...
        if self.upsample:
            ...
            if True:
                out = conv2d(input, weight, padding=0, stride=2, groups=batch)
            else:
            ...
            input = input.view(1, batch * in_channel, height, width)
            if True:
                out = conv2d(input, weight, padding=self.padding, groups=batch)
            else:
...
class ConvRenderBlock2d(nn.Module):
    ...
    def __init__(self, 
        ...
        if mode == 'original':
            ...
            self.blur = Blur(kernel=[1, 3, 3, 1], pad=(1, 1), upsample_factor=upsample_factor)
            self.skip_upsample = Upsample(kernel=[1, 3, 3, 1], factor=upsample_factor)
    ...
    def forward(self, x, skip=None):
        if self.mode == 'original':
            if self.upsample_factor != 1:
                x = torch.nn.functional.interpolate(x, scale_factor=self.upsample_factor, mode='nearest')
            x = self.conv(x)
        else:
            x = self.conv(x)
            x = self.blur(x)
        ...
        if skip is not None:
            if self.mode == 'original':
                skip = torch.nn.functional.interpolate(skip, scale_factor=self.upsample_factor, mode='bilinear', align_corners=False)
            else:
                skip = self.skip_upsample(skip)
            rgb = (rgb + skip) / math.sqrt(2)
        return x, rgb
...
class ModResBlock(nn.Module):
    ...
    def __init__(self, 
        ...
        if in_channel != out_channel:
            self.conv_s = EqualConv2d(in_channel,out_channel,kernel_size=1, stride = 1 , padding= 0, bias=False)
        # the first conv layer doesn't have bias because it is fused with the leakyReLU activation
        self.conv0 = ModulatedConv2d(in_channel, out_channel, kernel_size= 3, z_dim= z_dim, upsample= upsample_factor == 2)
        self.conv1 = ModulatedConv2d(out_channel, out_channel, kernel_size = 3, z_dim = z_dim, upsample= False)

        # self.activation = FusedLeakyReLU(out_channel, bias=True)
        self.toRGB = ModulatedConv2d(out_channel, 3, z_dim=z_dim, kernel_size=1,bias=True, activate=False)

    def forward(self, x, z, img=None):

        y = self.conv0(x, z)
        y = self.conv1(y, z)

        rgb = self.toRGB(y, z)
        if img is not None: # Skip mode
            if self.upsample_factor > 1:
                skip = self.skip_upsample(img)
            else:
                skip = rgb
            rgb = (rgb + skip) / math.sqrt(2)
        else: # Resner mode

...
import scipy.signal
import torch
# from lib.networks.reference.torch_utils import persistence
# from lib.networks.reference.torch_utils  import misc
from lib.networks.reference.torch_utils.ops  import upfirdn2d
from lib.networks.reference.torch_utils.ops  import grid_sample_gradfix
from lib.networks.reference.torch_utils.ops  import conv2d_gradfix
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images_list, debug_percentile=None):
        ...
        for k, images in enumerate(images_list):
            assert isinstance(images_list[k], torch.Tensor) and images_list[k].ndim == 4

        batch_size, _ , height, width = images_list[0].shape
        ...
        if G_inv is not I_3:
            ...
            for i, images in enumerate(images_list):
                ...
                G_inv_ = translate2d((mx0 - mx1) / 2, (my0 - my1) / 2) @ G_inv_

                # Upsample.
                images = upfirdn2d.upsample2d(x=images, f=self.Hz_geom, up=2)
                G_inv_ = scale2d(2, 2, device=device) @ G_inv_ @ scale2d_inv(2, 2, device=device)
                G_inv_ = translate2d(-0.5, -0.5, device=device) @ G_inv @ translate2d_inv(-0.5, -0.5, device=device)

                # Execute transformation.
                shape = [batch_size, num_channels, (height + Hz_pad * 2) * 2, (width + Hz_pad * 2) * 2]
                G_inv_ = scale2d(2 / images.shape[3], 2 / images.shape[2], device=device) @ G_inv_ @ scale2d_inv(2 / shape[3], 2 / shape[2], device=device)
                grid = torch.nn.functional.affine_grid(theta=G_inv_[:,:2,:], size=shape, align_corners=False)
        ...
        if C is not I_4:
            for i, images in enumerate(images_list):
                ...
                if num_channels == 3:
                    images = C_[:, :3, :3] @ images + C_[:, :3, 3:]
                elif num_channels == 1:
                    C_ = C_[:, :3, :].mean(dim=1, keepdims=True)
                    images = images * C_[:, :, :3].sum(dim=2, keepdims=True) + C_[:, :, 3:]
                elif num_channels == 6:
                    images[:, :3] = C_[:, :3, :3] @ images[:, :3] + C_[:, :3, 3:]
                    images[:, 3:] = C_[:, :3, :3] @ images[:, 3:] + C_[:, :3, 3:]
                else:
        ...
        if self.noise > 0:
            ...
            for i, images in enumerate(images_list):
                batch_size, num_channels, height, width = images.shape
                images_list[i] = images + torch.randn([batch_size, num_channels, height, width], device=device) * sigma

...
from ..utils.stylegan2_official_ops  import upfirdn2d, conv2d_gradfix, bias_act
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        **kwargs):        
        ...
        #? How to support non-square image
        min
        if isinstance(image_size, tuple):
            H, W = image_size[0], image_size[1]
             ...
             H = W = image_size
        self.H, self.W = H, W 
        if H in _RESOLUTIONS_ALLOWED:
            res_ = H
        else:
            res_ = pow(2, int(np.log2(min(H,W))) + 1)
        wh_ratio = round(max(W,H) / min(W,H))
        h_refine, w_refine = res_, wh_ratio * res_

        sh = init_res
        self.h_init, self.w_init = int(sh), int(wh_ratio * sh)

        self.model = StyleGAN2Discriminator(
            resolution= h_refine,
            wh_ratio= wh_ratio,
            init_res=init_res,
            # mbstd_groups = 4,
            conv_clamp = 256,
            fmaps_base=32 << 10,
            fmaps_max=max_channel)
        
        self.w_refine, self.h_refine = w_refine, h_refine



    def forward(self, image, data_type  = 'dis',mode = 'train', **kwargs):
        output = {}

        if self.h_refine != self.H:
            image = F.interpolate(image, (self.h_refine, self.w_refine))
        if cfg.is_debug:
            save_tensor_img(torchvision.utils.make_grid(image[:,:3].detach().cpu()), 'tmp', 'rgb_Din.jpg')
        # ws = self.mapping(z)
        # # a = ws.detach().cpu().numpy()
        # image = self.synthesis(ws)
        output = self.model(image, impl = 'cuda',fp16_res=64)
...
class StyleGAN2Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        #     resolution = pow(2, int(np.log2(min(resolution))) + 1)
        #     hw_ratio = round(max(W,H) / min(W,H))
            
        # self.H, self.W = H, W
        # self.h_refine, self.w_refine = resolution, hw_ratio * resolution
        ...
        for res_log2 in range(self.final_res_log2, self.init_res_log2 - 1, -1):
            ...
            if res_log2 == self.final_res_log2 or self.architecture == 'skip':
                ...
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/FromRGB/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/FromRGB/bias')
            ...
            if res != self.init_res:
                ...
                self.add_module(layer_name,
                                ConvLayer(in_channels=in_channels,
                                          ...
                                          conv_clamp=conv_clamp))
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/Conv0/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/Conv0/bias')
                ...
                self.add_module(layer_name,
                                ConvLayer(in_channels=in_channels,
                                          ...
                                          conv_clamp=conv_clamp))
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/Conv1_down/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/Conv1_down/bias')
            ...
            # Convolution block for last resolution.
                ...
                self.add_module(
                    ...
                    ConvLayer(in_channels=in_channels + mbstd_channels,
                              ...
                              conv_clamp=conv_clamp))
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/Conv/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/Conv/bias')

                # Second layer, as a fully-connected layer.
                layer_name = f'layer{2 * block_idx + 1}'
                self.add_module(layer_name,
                                DenseLayer(in_channels=in_channels * res * res *  wh_ratio,
                                           out_channels=in_channels,
                                           add_bias=True,
                                           init_bias=0.0,
                                           use_wscale=use_wscale,
                                           ...
                                           activation_type='lrelu'))
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/Dense0/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/Dense0/bias')
    ...
    def forward(self, image, label=None, fp16_res=4, impl='cuda'):
        # Check shape.
        expected_shape = (self.image_channels, self.resolution, self.resolution * self.wh_ratio)
        if image.ndim != 4 or image.shape[1:] != expected_shape:
            raise ValueError(f'The input tensor should be with shape '
                             f'[batch_size, channel, height, width], where '
                             f'`channel` equals to {self.image_channels}, '
                             f'`height`, `width` equal to {self.resolution},{self.resolution * self.wh_ratio}!\n'
                             f'But `{image.shape}` is received!')
        # if self.H != self.h_refine or self.W != self.w_refine:
        #     image = F.interpolate(image, (self.h_refine, self.w_refine))
        # if cfg.is_debug:
        #     save_tensor_img(torchvision.utils.make_grid(image[:,:3].detach().cpu()), 'tmp', 'rgb_Din.jpg')
        if self.label_dim > 0:

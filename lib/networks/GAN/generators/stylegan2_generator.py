...
from ..utils.stylegan2_official_ops import fma
from ..utils.stylegan2_official_ops import bias_act
from ..utils.stylegan2_official_ops import upfirdn2d
from ..utils.stylegan2_official_ops import conv2d_gradfix
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        **kwargs):        
        ...
        self.h_refine = h_refine

        sh = init_res
        self.h_init, self.w_init = int(sh), int(wh_ratio * sh)

        self.model = StyleGAN2Generator(
            resolution= h_refine,
            wh_ratio= wh_ratio,
            init_res=4,
            mapping_layers=8,
    ...
    def forward(self, raw_batch, data_type  = 'gen',mode = 'train'):
        ...
        image = self.model(z,
                           impl = 'cuda',
                            noise_mode= 'random' if mode == 'train' else 'const' ,
                            w_moving_decay = 0.995,
                            style_mixing_prob = 0.9,
                           fp16_res = 64)['image']
        
        if self.H != self.h_refine:
...
class SynthesisNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if res == init_res:
                if self.const_input:
                    ...
                    self.pth_to_tf_var_mapping['early_layer.const'] = (
                        f'{res}x{res * wh_ratio}/Const/const')
                else:
                    channels = out_channels * res * res * wh_ratio
                    self.add_module('early_layer',
                    ...
                    self.pth_to_tf_var_mapping['early_layer.weight'] = (
                        f'{res}x{res * wh_ratio}/Dense/weight')
                    self.pth_to_tf_var_mapping['early_layer.bias'] = (
                        f'{res}x{res * wh_ratio}/Dense/bias')
            else:
                ...
                if self.architecture == 'resnet':
                    ...
                    self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                        f'{res}x{res * wh_ratio}/Skip/weight')
            ...
            self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                f'{res}x{res * wh_ratio}/{tf_layer_name}/weight')
            self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                f'{res}x{res * wh_ratio}/{tf_layer_name}/bias')
            self.pth_to_tf_var_mapping[f'{layer_name}.style.weight'] = (
                f'{res}x{res * wh_ratio}/{tf_layer_name}/mod_weight')
            self.pth_to_tf_var_mapping[f'{layer_name}.style.bias'] = (
                f'{res}x{res * wh_ratio}/{tf_layer_name}/mod_bias')
            self.pth_to_tf_var_mapping[f'{layer_name}.noise_strength'] = (
                f'{res}x{res * wh_ratio}/{tf_layer_name}/noise_strength')
            self.pth_to_tf_var_mapping[f'{layer_name}.noise'] = (
                f'noise{2 * block_idx}')
            ...
            if res_log2 == self.final_res_log2 or self.architecture == 'skip':
                ...
                self.add_module(layer_name,
                                ModulateConvLayer(in_channels=out_channels,
                                                  ...
                                                  eps=eps))
                self.pth_to_tf_var_mapping[f'{layer_name}.weight'] = (
                    f'{res}x{res * wh_ratio}/ToRGB/weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.bias'] = (
                    f'{res}x{res * wh_ratio}/ToRGB/bias')
                self.pth_to_tf_var_mapping[f'{layer_name}.style.weight'] = (
                    f'{res}x{res * wh_ratio}/ToRGB/mod_weight')
                self.pth_to_tf_var_mapping[f'{layer_name}.style.bias'] = (
                    f'{res}x{res * wh_ratio}/ToRGB/mod_bias')
...
class InputLayer(nn.Module):
    ...
    def __init__(self, init_res, wh_ratio, channels):
        super().__init__()
        self.const = nn.Parameter(torch.randn(1, channels, init_res, init_res * wh_ratio))
